...
function ServiceSettingsPanel({
  ...
  return <section className="configuration-panel" aria-labelledby="configuration-services-title">
    ...
    <WriteAction>
      ...
      <section id="configuration-connections" tabIndex={-1}><header><h3>Servizi richieste, ricerca e download</h3><p>Prowlarr gestisce ricerca e invio al proprio client torrent; i profili locali restano disponibili per monitoraggio e funzioni future.</p></header><div className="service-connection-grid"><ServiceConnectionCard title="Jellyseerr" description="Gestione e stato delle richieste." value={currentDraft.connections.jellyseerr} configured={services.connections.jellyseerr.api_key_configured} onChange={(value) => updateConnection("jellyseerr", value)} /><ServiceConnectionCard title="Prowlarr" description="Indexer API principale e instradamento download." value={currentDraft.connections.prowlarr} configured={services.connections.prowlarr.api_key_configured} onChange={(value) => updateConnection("prowlarr", value)} /><ServiceConnectionCard title="Jackett" description="Indexer API alternativo; l’invio diretto non è gestito da Prowlarr." value={currentDraft.connections.jackett} configured={services.connections.jackett.api_key_configured} onChange={(value) => updateConnection("jackett", value)} /></div><TorrentClientsSettings value={currentDraft.connections.torrent_clients} snapshot={services.connections.torrent_clients || []} onChange={(torrent_clients) => updateConnections({ ...currentDraft.connections, torrent_clients })} /></section>
