...
function ServiceCatalogSettings({
  ...
  return <section id="configuration-catalogs" className="service-form-grid service-form-grid--integrations" tabIndex={-1}>
    ...
    <div className="service-credential-field"><label>Client Secret Trakt<input type="password" disabled={traktDeviceBusy || trakt.clear_client_secret} placeholder={snapshot.trakt.client_secret_configured ? "Lascia vuoto per conservarlo" : "Client Secret"} value={trakt.client_secret || ""} onChange={(event) => onTraktChange({ ...trakt, client_secret: event.target.value, clear_client_secret: false })} /></label><SavedCredentialControl configured={snapshot.trakt.client_secret_configured} pendingRemoval={Boolean(trakt.clear_client_secret)} label="Rimuovi Client Secret salvato" disabled={traktDeviceBusy} onPendingRemovalChange={(clear_client_secret) => onTraktChange({ ...trakt, clear_client_secret, client_secret: "" })} /></div>
