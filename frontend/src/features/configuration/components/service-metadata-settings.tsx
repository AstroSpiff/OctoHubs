...
function ServiceMetadataSettings({
  ...
  return <section id="configuration-metadata" className="service-form-grid" tabIndex={-1}>
    ...
    <div className="service-credential-field"><label>API Key TMDB<input type="password" disabled={connections.tmdb.clear_api_key} placeholder={snapshot.tmdb.api_key_configured ? "Lascia vuota per conservarla" : "API Key"} value={connections.tmdb.api_key || ""} onChange={(event) => update("tmdb", { api_key: event.target.value, clear_api_key: false })} /></label><SavedCredentialControl configured={snapshot.tmdb.api_key_configured} pendingRemoval={Boolean(connections.tmdb.clear_api_key)} label="Rimuovi API Key salvata" onPendingRemovalChange={(clear_api_key) => update("tmdb", { clear_api_key, api_key: "" })} /></div>
    ...
    <div className="service-credential-field"><label>API Keys MDBList <small>{snapshot.mdblist.api_keys_configured ? `${snapshot.mdblist.api_keys_configured} configurate` : "Nessuna configurata"}</small><textarea rows={3} disabled={connections.mdblist.clear_api_keys} placeholder="Una per riga o separate da virgole" value={mdblistKeys} onChange={(event) => { onMdblistKeysChange(event.target.value); update("mdblist", { clear_api_keys: false }); }} /></label><SavedCredentialControl configured={snapshot.mdblist.api_keys_configured > 0} pendingRemoval={Boolean(connections.mdblist.clear_api_keys)} label="Rimuovi le API Keys salvate" onPendingRemovalChange={(clear_api_keys) => update("mdblist", { clear_api_keys })} /></div>
    <div className="service-credential-field"><label>API Keys OMDb <small>{snapshot.omdb.api_keys_configured ? `${snapshot.omdb.api_keys_configured} configurate` : "Nessuna configurata"}</small><textarea rows={3} disabled={connections.omdb.clear_api_keys} placeholder="Una per riga o separate da virgole" value={omdbKeys} onChange={(event) => { onOmdbKeysChange(event.target.value); update("omdb", { clear_api_keys: false }); }} /></label><SavedCredentialControl configured={snapshot.omdb.api_keys_configured > 0} pendingRemoval={Boolean(connections.omdb.clear_api_keys)} label="Rimuovi le API Keys salvate" onPendingRemovalChange={(clear_api_keys) => update("omdb", { clear_api_keys })} /></div>
  </section>;
}
