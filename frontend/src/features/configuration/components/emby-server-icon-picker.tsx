...
function EmbyServerIconPicker({
  ...
  return <div className="configuration-icon-picker">
    <span className="configuration-icon-preview" title={icon}><PreviewIcon size={20} color={color} aria-hidden="true" /></span>
    ...
    <div className="configuration-icon-colors" aria-label="Colori rapidi">{iconColorPresets.map((preset, index) => <button key={preset} type="button" className={`configuration-icon-color-${index + 1}${preset.toLowerCase() === color.toLowerCase() ? " is-active" : ""}`} title={`Usa ${preset}`} aria-label={`Usa colore ${preset}`} aria-pressed={preset.toLowerCase() === color.toLowerCase()} disabled={disabled} onClick={() => onColorChange(preset)} />)}</div>
  </div>;
}
