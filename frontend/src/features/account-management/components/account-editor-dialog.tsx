...
function AccountEditorDialog({ account, busy, error, onClose, onCreate, onUpdate, open }: AccountEditorDialogProps) {
  ...
  return (
    <DialogBackdrop className="account-dialog-backdrop" dismissible={!busy} onDismiss={onClose}>
      <form className="account-editor-dialog" onSubmit={(event) => void submit(event)} role="dialog" aria-modal="true" aria-labelledby="account-editor-title">
        ...
        <footer><Button type="button" variant="ghost" onClick={onClose} disabled={busy}>Annulla</Button><Button type="submit" variant="primary" disabled={busy || (creating && (!username.trim() || password.length < 8 || repeatPassword.length < 8))}>{busy ? "Salvataggio..." : creating ? "Crea account" : "Salva modifiche"}</Button></footer>
      </form>
    </DialogBackdrop>
  );
}
