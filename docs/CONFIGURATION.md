...
## DATABASE (app data)
Controls PostgreSQL storage for app data (not auth users).
...
Note: the auth database for users is configured via `AUTH_DATABASE_URL` in the environment, not in `config.json`.
